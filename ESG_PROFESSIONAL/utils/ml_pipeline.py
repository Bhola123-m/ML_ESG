...
def compute_sentiment(df):
    ...
    bias = {'Technology':0.3,'Healthcare':0.2,'Consumer':0.1,'Finance':-0.1,'Energy':-0.35}
...
def run_lstm(X, y):
    m = MLPClassifier(hidden_layer_sizes=(256,128,64), activation='tanh',
                       ...
                       learning_rate_init=0.001, early_stopping=True,
                       n_iter_no_change=25, class_weight='balanced' if hasattr(MLPClassifier, 'class_weight') else None)
    # class_weight not supported — remove it safely
    m = MLPClassifier(hidden_layer_sizes=(256,128,64), activation='tanh',
                       solver='adam', max_iter=700, random_state=42,
                       alpha=0.001, learning_rate='adaptive',
                       learning_rate_init=0.001, early_stopping=True, n_iter_no_change=25)
    scores = cross_val_score(m, X, y, cv=CV, scoring='accuracy')
    m.fit(X, y); return scores.mean(), scores.std(), m
