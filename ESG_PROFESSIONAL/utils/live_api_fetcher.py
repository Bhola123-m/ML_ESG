...
def create_fallback(ticker, sector):
    ...
    defaults = {
        'Technology': {'esg': 65, 'mktcap': 50e9, 'pm': 15, 'beta': 1.2, 'dte': 40},
        'Finance': {'esg': 55, 'mktcap': 30e9, 'pm': 22, 'beta': 1.1, 'dte': 350},
        'Healthcare': {'esg': 68, 'mktcap': 25e9, 'pm': 12, 'beta': 0.7, 'dte': 80},
        'Energy': {'esg': 35, 'mktcap': 20e9, 'pm': 8, 'beta': 1.0, 'dte': 60},
        'Consumer': {'esg': 62, 'mktcap': 15e9, 'pm': 7, 'beta': 0.9, 'dte': 100},
    }
    
    d = defaults.get(sector, defaults['Technology'])
...
def fetch_live_dataset(api_keys=None, progress_callback=None, max_companies=550):
    ...
    # Use pre-configured API key if not provided
    if not api_keys:
    ...
    if mask.sum() > 0:
        for idx in df[mask].index:
            ...
            base_esg = {'Technology': 65, 'Finance': 55, 'Healthcare': 68, 'Energy': 35, 'Consumer': 62}[sector]
            ...
            df.loc[idx, 'esg_score'] = np.clip(base_esg, 10, 95)
