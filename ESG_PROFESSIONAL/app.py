...
# Pandas version-safe month-end frequency (ME introduced in 2.2.0, older uses M)
_pd_ver = tuple(int(x) for x in pd.__version__.split('.')[:2])
MONTH_FREQ = 'ME' if _pd_ver >= (2, 2) else 'M'
...
def step5():
    ...
    # 7. ESG Score vs Risk Score (with LOWESS trendline — safe fallback)
    st.markdown('<div class="sec-title">7. ESG Score vs Risk Score Relationship</div>',unsafe_allow_html=True)
    try:
        import statsmodels.api as sm  # noqa: F401 — check availability before plotly calls it
        fig7=px.scatter(df.sample(min(280,len(df)),random_state=42),
                         x='esg_score',y='risk_score',
                         color='risk_label',color_discrete_map=PALETTE,
                         hover_name='company_name',hover_data=['sector'],
                         trendline='lowess',opacity=0.7,
                         labels={'esg_score':'ESG Score','risk_score':'Risk Score'})
    except Exception:
        # statsmodels not available — render chart without trendline
        fig7=px.scatter(df.sample(min(280,len(df)),random_state=42),
                         x='esg_score',y='risk_score',
                         color='risk_label',color_discrete_map=PALETTE,
                         hover_name='company_name',hover_data=['sector'],
                         trendline=None,opacity=0.7,
                         labels={'esg_score':'ESG Score','risk_score':'Risk Score'})
    ...
    dates=pd.date_range(start=datetime.now(),periods=6,freq=MONTH_FREQ)
    fig11=go.Figure()
    ...
    # 13. ML Model Performance Summary
